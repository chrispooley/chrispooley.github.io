...
function generate_plots()
{
	...
	case "OP_AGE_MATRIX": case "OP_AGE_MATRIX_POST": case "OP_AGE_MATRIX_DIF": break;
	...
	case "OP_ME": case "OP_AGE_MATRIX":	case "OP_AGE_MATRIX_POST": case "OP_AGE_MATRIX_DIF":	
...
function plot_results()
{
	...
	case "OP_AGE_MATRIX": case "OP_AGE_MATRIX_POST": drawmatrix(vis); break;
	case "OP_AGE_MATRIX_DIF": drawmatrix_dif(vis); break;
	case "OP_PARAM_TABLE": case "OP_PRIOR_TABLE": break;
...
/// Draws a matrix giving difference between posterior and specified age mixing matrices
function drawmatrix_dif(vis)
{
	plottype = "matrix";
	...
	for(i = 0; i < nage; i++){
		for(j = 0; j < nage; j++){
			var val = Math.log(vis.ele[i][j]);
			if(val > 0){
				cv.globalAlpha = val/max;
				fillrect(i*d,j*d,d,d,RED); 
			}
			else{
				cv.globalAlpha = -val/max;
				fillrect(i*d,j*d,d,d,BLUE); 
			}			
		}
	}
	cv.globalAlpha = 1;
