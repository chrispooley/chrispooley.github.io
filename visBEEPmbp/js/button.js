function buttoninit()                                      // Initialises the buttons used on pages
{
 ...
 /*
	addbutton("Transmission",0,y,menux,ddy,TABBUT,TABBUT,R_PAGE,-1); y += ddy; y += dby;
	
	nout = 0; for(cl = 0; cl < ncla-2; cl++){ out[nout] = cla[cl].name; nout++;}
	addsub();
	
	addbutton("Data Tables",0,y,menux,ddy,TABBUT,TABBUT,DATA_PAGE,-1); y += ddy; y += dby;
 
	addbutton("State Outputs",0,y,menux,ddy,TABBUT,TABBUT,STATE_PAGE,-1); y += ddy; y += dby;
 
	addbutton("Model Parameters",0,y,menux,ddy,TABBUT,TABBUT,STATE_PAGE,-1); y += ddy; y += dby;
 
	addbutton("Diagnostics",0,y,menux,ddy,TABBUT,TABBUT,STATE_PAGE,-1); y += ddy; y += dby;
	*/
 ...
 /*
    ...
    addbutton("",0,20,0,0,-1,LOGOBUT,-1,-1);
...
function changepage(pagenew, s, ss)                        // Changes the page
{
	...
	/*
	if(selectbub != -1){ 
		buboff(1); // This has been changed
		if(selectbub != -1) return;
	}
	*/
...
function verticaltext(text,x,y,font,col)   // Draws centered text at an angle
{
	text = text.toString();
	cv.font = font;	
	cv.save();
...
function drawgradientline(x1, y1, x2, y2, f1, f2, wid)         // Draws a line with a colour gradient
{
	...
	v = Math.floor(f1*255); grad.addColorStop(0, "rgb("+v+","+0+","+(255-v)+")");
	v = Math.floor(f2*255); grad.addColorStop(1, "rgb("+v+","+0+","+(255-v)+")");
