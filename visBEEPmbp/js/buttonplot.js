...
{ 
	...
	case PAGESUBSUBBUT:
		...
		plottext(text,x+1,y+16+off-2,font,col);
		break;
	...
	case PARAGRAPHBUT:
		...
		for(j = 0; j < nlines; j++) plottext(lines[j],x+5,y+18+linesy[j],HELPFONT,BLACK);
		break;
		
	case SLIDEPARAGRAPHBUT:
		alignparagraph(text,dx);
		nlines_disp = Math.floor(dy/lineheight);
		jmin = Math.floor(slidey*nlines);
